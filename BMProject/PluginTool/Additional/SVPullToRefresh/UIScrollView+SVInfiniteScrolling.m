...
//        NSLog(@"旧：%f, 新:%f", [[change valueForKey:NSKeyValueChangeOldKey] CGPointValue].y, [[change valueForKey:NSKeyValueChangeNewKey] CGPointValue].y);
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVInfiniteScrollingStateLoading && self.enabled && !self.exclusive) {
        CGFloat scrollViewContentHeight = self.scrollView.contentSize.height;
        CGFloat scrollOffsetThreshold = scrollViewContentHeight-self.scrollView.bounds.size.height;
        
        if(!self.scrollView.isDragging && self.state == SVInfiniteScrollingStateTriggered){
            self.state = SVInfiniteScrollingStateLoading;
            ((UIScrollView *)self.superview).pullToRefreshView.exclusive = YES;
        }
            ...
            self.state = SVInfiniteScrollingStateStopped;
            ((UIScrollView *)self.superview).pullToRefreshView.exclusive = NO;
        }
    }
}
